...
void calc_outputs(Output *out, Errors e, float thrust, Angles theta);
...
void main(void) {
	...
	Positions pos, posdot, posdotdot  = {0, 0, 0};
// Declare Output struct
	...
	while(1)
	{
		...
		posdotdot.Z = sensor.az;
		pos.Z = sensor.alt;

		pid_controller(sensor, &out, &thetadot, &theta, &thetaInt, &pos,  &posdot, &posdotdot,  e);
...
void pid_controller(Sensor sensor, Output *outs, Angles *thetadot, Angles *theta, Angles *thetaInt, Errors e, Positions *pos, Positions *posdot, Positions *posdotdot)
{
...
// Calculate all errors for angular velocity
	...
	e.Z = (KI_Z * (z_target - pos->Z)) + (KD_Z * (zdot_target - posdot->Z)) + (KD_Z * K * (zdotdot_target - posdotdot->Z) / MASS); // error for positional accel/velocity
// Calculate all outputs (motor rpm -> esc voltage)
	calc_outputs(outs, e, thrust, *theta);
...
void calc_outputs(Output *out, Errors e, float thrust, Angles theta)
{
	thrust += MASS*e.Z/(K*cos(theta.angX + theta.angY));
